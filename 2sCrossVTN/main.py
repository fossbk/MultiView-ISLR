...
from trainer.gsam import GSAM, LinearScheduler
...
if __name__ == "__main__":
    ...
    if num_folds > 1:
        ...
        for idx,(train_idx, val_idx) in enumerate(kfold.split(signer)):
            ...
            if cfg['training'].get('use_gsam', False):
                lr_scheduler = LinearScheduler(
                    T_max=cfg['training']['total_epoch'] * len(train_loader),
                    max_value=cfg['training']['learning_rate'],
                    min_value=cfg['training']['learning_rate'] * 0.01,
                    optimizer=optimizer
                )
                rho_scheduler = LinearScheduler(
                    T_max=cfg['training']['total_epoch']*len(train_loader),
                    max_value=cfg['training']['rho_max'],
                    min_value=cfg['training']['rho_min']
                )
                # Wrap base optimizer thành GSAM optimizer:
                optimizer = GSAM(
                    params=model.parameters(),
                    base_optimizer=optimizer,
                    model=model,
                    gsam_alpha=cfg['training']['gsam_alpha'],
                    rho_scheduler=rho_scheduler,
                    adaptive=cfg['training'].get('gsam_adaptive', False)
                )
