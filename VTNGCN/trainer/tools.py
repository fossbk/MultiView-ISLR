...
class MyCustomLoss(nn.Module):
    def __init__(self, reduction=None, label_smoothing=0, cls_x=1, cts_x=1, cosine_x=1, cv_align_weights=0.1, margin=0.5):
        ...
        self.cv_align_weights = cv_align_weights
        self.margin = margin

    def pairwise_contrastive_loss(self, features_a, features_b, margin=0.5):
        B = features_a.size(0)
        a = F.normalize(features_a.flatten(1, 2), p=2, dim=-1)
        b = F.normalize(features_b.flatten(1, 2), p=2, dim=-1)
        similarity = torch.matmul(a, b.T)
        positive_mask = torch.eye(B, device=similarity.device).bool()
        loss_positive = torch.sum((1 - similarity[positive_mask]) ** 2)
        negative_mask = ~positive_mask
        loss_negative = torch.sum(torch.clamp(similarity[negative_mask] - margin, min=0) ** 2)
        total_loss = (loss_positive + loss_negative) / (B ** 2)
        return total_loss
    ...
    def forward(self, logits=None, labels=None, student_features=None, teacher_features=None, 
                student_logits=None, teacher_logits=None, rgb_left_ft=None, rgb_center_ft=None, 
                rgb_right_ft=None, visual_fusion_ft=None, gloss_visual_fusion_ft=None, **kwargs):
        loss = 0
        loss_dict = {}
        ...
        if rgb_left_ft is not None and rgb_center_ft is not None and rgb_right_ft is not None:
            loss_center_left = self.pairwise_contrastive_loss(rgb_center_ft, rgb_left_ft, self.margin)
            loss_center_right = self.pairwise_contrastive_loss(rgb_center_ft, rgb_right_ft, self.margin)
            loss_left_right = self.pairwise_contrastive_loss(rgb_left_ft, rgb_right_ft, self.margin)
            total_contrastive_loss = (loss_center_left + loss_center_right + loss_left_right) / 3
            loss += self.cv_align_weights * total_contrastive_loss
            loss_dict.update({
                'contrastive_loss': total_contrastive_loss.item(),
                'center_left_loss': loss_center_left.item(),
                'center_right_loss': loss_center_right.item(),
                'left_right_loss': loss_left_right.item()
            })
